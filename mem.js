const cardsArray = [
    {
        name:'dog',
        icon:'<i class="fa-solid fa-dog"></i>'
    },
    {
        name:'hippo',
        icon:'<i class="fa-solid fa-hippo"></i>'
    },
    {
        name:'fish',
        icon:'<i class="fa-solid fa-fish"></i>'
    },
    {
        name:'cat',
        icon:'<i class="fa-solid fa-cat"></i>'
    },
    {
        name:'spider',
        icon:'<i class="fa-solid fa-spider"></i>'
    },
    {
        name:'frog',
        icon:'<i class="fa-solid fa-frog"></i>'
    },
    {
        name:'dog',
        icon:'<i class="fa-solid fa-dog"></i>'
    },
    {
        name:'hippo',
        icon:'<i class="fa-solid fa-hippo"></i>'
    },
    {
        name:'fish',
        icon:'<i class="fa-solid fa-fish"></i>'
    },
    {
        name:'cat',
        icon:'<i class="fa-solid fa-cat"></i>'
    },
    {
        name:'spider',
        icon:'<i class="fa-solid fa-spider"></i>'
    },
    {
        name:'frog',
        icon:'<i class="fa-solid fa-frog"></i>'
    }
];

let flippedCards = [];
let matchedPairs = 0;
let timerInterval;
let seconds=0;
let score = 0;
shuffleCards();
const gameBoard = document.getElementById('gameBoard');
const scoreDisplay = document.getElementById('score'); 
const timerDisplay = document.getElementById('timer');
const restartButton = document.getElementById('restartButton');
displayCards();
startTimer();

restartButton.addEventListener('click', () => {
    restartGame(); // Call the restartGame function when the button is clicked
});

function restartGame() {
    
    flippedCards = [];
    matchedPairs = 0;
    seconds = 0;
    score = 0;

    // Clear the game board content
    gameBoard.innerHTML = '';

    // Restart the timer and display
    stopTimer();
    startTimer();
    timerDisplay.textContent = `Time: ${seconds} seconds`;

    // Reset the score display
    scoreDisplay.textContent = `Score: ${score}`;

    // Shuffle and display the cards again
    shuffleCards();
    displayCards();

    // Remove any additional classes added during game over or win state
    gameBoard.classList.remove('game-over', 'won');
    gameBoard.classList.add('game');
}
  
function shuffleCards(){
    for(let i=cardsArray.length-1;i>=0;i--){
        const randIndex = Math.floor(Math.random()*(i+1));
        [cardsArray[i],cardsArray[randIndex]] = [cardsArray[randIndex],cardsArray[i]]
    }
}

function displayCards(){
    cardsArray.forEach((curr,index,arr)=>{
        const card = document.createElement('div');
        card.setAttribute('id',index); 
        card.classList.add('cardback');
        card.classList.add('active');
        gameBoard.append(card); 
        card.addEventListener('click',flipCard);
    })
}

function flipCard(){
    if(flippedCards.length<2 && this.classList.contains('active')){
        let cardId = this.getAttribute('id');
        if (flippedCards.includes(this)) {
            return; 
        }
        flippedCards.push(this);
        this.classList.remove('cardback');
        this.innerHTML = cardsArray[cardId].icon;
        if(flippedCards.length==2){
            setTimeout(checkMatch,800);
        }
    }
}

function checkMatch(){
    const card1Id = flippedCards[0].getAttribute('id');
    const card2Id = flippedCards[1].getAttribute('id');
    if(cardsArray[card1Id].name === cardsArray[card2Id].name){
        flippedCards[0].style.border = 'none';
        flippedCards[0].style.backgroundColor = '#c5d2c3';
        flippedCards[0].innerHTML = '';
        flippedCards[0].classList.remove('active');
        flippedCards[1].classList.remove('active');
        flippedCards[1].style.border = 'none';
        flippedCards[1].style.backgroundColor = '#c5d2c3';
        flippedCards[1].innerHTML = "";
        updateScore();
        matchedPairs++;
        checkGameOver();
    }
    else{
        flippedCards[0].innerHTML = '';
        flippedCards[0].classList.add('cardback');
        flippedCards[1].innerHTML = '';
        flippedCards[1].classList.add('cardback');
    }
    flippedCards = [];
}

function checkGameOver(){
    if(matchedPairs == cardsArray.length/2){
        while(gameBoard.firstChild){
            gameBoard.removeChild(gameBoard.firstChild)
        }
        gameBoard.innerHTML = `You Won!!!! ${seconds} seconds`;
        stopTimer();
        gameBoard.classList.remove('game');
        gameBoard.classList.add('won');
    }
}

function startTimer() {
    timerInterval = setInterval(updateTimer, 1000); 
}

function updateTimer() {
    seconds++;
    const timerDisplay = document.getElementById('timer');
    timerDisplay.textContent = `Time: ${seconds} seconds`;
    if (seconds >= 60) { 
        stopTimer();
        gameBoard.innerHTML = `Time is up! Game Over and your score is ${score}`;
        
        gameBoard.classList.remove('game');
        gameBoard.classList.add('game-over');
    }
}

function stopTimer() {
    clearInterval(timerInterval);   
}

function updateScore() {
    score += 10; // Increase the score by 10 for each matched pair
    scoreDisplay.textContent = `Score: ${score}`;
}